# E-Ponto

O **E-Ponto** é uma aplicação de gestão de ponto eletrônico, desenvolvida para facilitar o controle de jornada de trabalho dos colaboradores. Com funcionalidades intuitivas e integração com banco de dados PostgreSQL, o sistema permite marcações como entrada, pausa, retorno e saída de forma eficiente. A aplicação foi projetada para ser simples, responsiva e fácil de usar.

## Demonstração
Acesse a aplicação: [e-ponto-eight.vercel.app](https://e-ponto-delta.vercel.app/)

---

## Funcionalidades

- **Registro de Ponto:** Entrada, pausa, retorno e saída com controle automatizado.
- **Envio de Emails:** Notificações automáticas por email para usuários e administradores.
- **Sistema de Login:** Controle de acesso seguro com autenticação.
- **Gerenciamento de Usuários:** Cadastro de usuários, permissões de administrador e controle de acesso.
- **Banco de Dados PostgreSQL:** Dados armazenados e gerenciados com segurança.
- **Migrações do Banco de Dados:** Estrutura do banco facilmente atualizável.
- **Deploy na Vercel:** Sistema online com atualizações frequentes.

---

## Tecnologias Utilizadas

- **Backend:** Flask (Python)
- **Banco de Dados:** PostgreSQL
- **Frontend:** HTML, CSS e JavaScript
- **Hospedagem:** Vercel
- **Gerenciamento de Dependências:** Pip
- **Envio de Emails:** Flask-Mail

---

## Configuração do Projeto

### Pré-requisitos

- Python 3.9+
- PostgreSQL
- Gerenciador de pacotes `pip`

### Passo a Passo

1. **Clone o repositório:**
   ```bash
   git clone https://github.com/sharktank-tech/e-ponto
   cd e-ponto
2. **Crie o ambiente virtual:**
   ```bash
    python3 -m venv .venv
3. **Atuve o ambiente:**
   ```bash
    source .venv/bin/activate
4. **Acesse sua aplicação:**
   http://127.0.0.0:8000 
